...
{
    public GameObject author_panel,scrollcontent;
    public Dropdown dropDownAuthor;
    public Text result1;
    ...
    public void PushLoadButton()
    {
        ...
        {
            GameObject prefab = (GameObject)Instantiate (author_panel);
            prefab.transform.SetParent (scrollcontent.transform, false); 
            GameObject toggle = prefab.transform.GetChild(0).gameObject;
            GameObject label = toggle.transform.GetChild(1).gameObject;

            if(saveData.NameTextList[i] == "作家名")
            {
                //作家名
                label.GetComponent<Text>().text="全て";
            }
            else
            {
                //作家名
                label.GetComponent<Text>().text=saveData.NameTextList[i];
            }
            
        }
        if (dropDownAuthor) 
        {
            dropDownAuthor.ClearOptions();    //現在の要素をクリアする
            List<string> list = new List<string>();
            for(int i=0; i<saveData.TitleTextList.Count; i++) {
                if (i == 0) {
                    list.Add ("全て");
                } else {
                    list.Add (saveData.NameTextList[i]);
                }
            }
            dropDownAuthor.AddOptions(list);  //新しく要素のリストを設定する
            dropDownAuthor.value = 0;         //デフォルトを設定(0～n-1)
        }
    }
    ...
    public void OnClick()
    {
        ...
        ddtmp = GameObject.Find("author").GetComponent<Dropdown>();
    ...
    public void OnClick2()
    {
        ...
        //Debug.Log(selected_year1_value +"." + selected_month1_value +"." +selected_day1_value);
        //Debug.Log(selected_year2_value +"." + selected_month2_value +"." +selected_day2_value);
        ...
        for (int i = 0; i<ChildObject.Length; i++)
        {
            ...
            if(realday > intday1)  // 2022.8.19 (2022.8.22)
            {
                ...
                {
                    Debug.Log("10あり");
                    if(ChildObject[i].transform.GetChild(2).gameObject.GetComponent<Text>().text != selectedvalue && selectedvalue != "全て")
                    {
                    ...
                    else if (ChildObject[i].transform.GetChild(2).gameObject.GetComponent<Text>().text == selectedvalue)
                    {
                        ChildObject[i].SetActive(true);
                    }
                }
